...
class Household:
    def __init__(self, charging_station: EV_Charger, ev: EV, ev_weekly_profile: list[dict], timescale: int = 15):
        self.ev = ev
        self.charging_station = charging_station
        self.ev_weekly_profile = self._normalize_ev_profile(ev_weekly_profile, timescale)
        self.timescale = timescale
        self.ev_is_home = True

    def _normalize_ev_profile(self, weekly_profile: list[dict], step_minutes: int) -> list[dict]:
        """
        ...
        for day, data in weekly_profile.items():
            ...
            for trip in trips:
                ...
                # Some data checks
                assert trip['departure'] < trip['arrival'], f"Departure time {trip['departure']} must be less than arrival time {trip['arrival']}"
                assert trip['arrival'] <= 2359, f"Arrival time {trip['arrival']} must be less than or equal to 2359"
                assert trip['departure'] >= 0, f"Departure time {trip['departure']} must be greater than or equal to 0"
    ...
    def step(self, actions, charge_price, discharge_price, sim_timestamp):
        ...
        if self.ev_is_home:
            self.charging_station.evs_connected[1] = self.ev # Assuming only one EV per household currently
        else: 
            self.charging_station.evs_connected[1] = None
        
        self.charging_station.step(actions, charge_price, discharge_price, sim_timestamp)
